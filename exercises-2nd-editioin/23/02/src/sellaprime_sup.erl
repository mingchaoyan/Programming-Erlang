...
set_new_report_handler() ->
    ...
    Now = get_now_str(),
    LogFile = lists:concat([LogPath, re:replace(Now, " ", "_", [global, {return, list}]), LogPathSuffix]),
    error_logger:delete_report_handler(logger_h), %% 移除旧的
    error_logger:add_report_handler(logger_h, LogFile). %% 设置新的

get_now_str() -> 
    {{Year, Month, Day}, {Hour, Min, Sec}} = erlang:localtime(),
    MonthPadding = io_lib:format("~2..0B",[Month]),
    DayPadding = io_lib:format("~2..0B",[Day]),
    HourPadding = io_lib:format("~2..0B",[Hour]),
    MinPadding = io_lib:format("~2..0B",[Min]),
    SecPadding = io_lib:format("~2..0B",[Sec]),
    lists:concat([Year, '-', MonthPadding, '-', DayPadding, ' ', HourPadding, '-', MinPadding, '-', SecPadding]).
